...
BOOLEAN HandleSoldierDeath( SOLDIERTYPE *pSoldier , BOOLEAN *pfMadeCorpse )
{
	...
	*pfMadeCorpse = FALSE;
	...
	if ( pSoldier->stats.bLife == 0 && !( pSoldier->flags.uiStatusFlags & SOLDIER_DEAD )	)
	{
					...
					if ( MercPtrs[ ubAttacker ]->bTeam == gbPlayerNum )
					{
						...
						switch(pSoldier->ubSoldierClass)
						{
							...
							case SOLDIER_CLASS_ELITE: {
								...
								//If the elite we or someone else killed was nabove 10 hp , it will multiply, GOOD LUCK!
								if (pSoldier->bOldLife > 10)
									for (int i = 0; i < 2; i++) { //We want to spawn two clones with half the hp of the previous one


										//SOLDIERCREATE_STRUCT assignment operator will simply copy everything from the previous soldier
										SOLDIERCREATE_STRUCT CloneStruct;
										UINT8 iNewIndex;

										CloneStruct = *pSoldier;
										//We have to set life to half because assignment operator copied life of the dead soldier and that is ZERO and wont work OBVIOUSLY LOL
										CloneStruct.bLife = pSoldier->stats.bLifeMax * 0.5f;
										CloneStruct.bLifeMax = CloneStruct.bLife;
										SOLDIERTYPE* ClonedSoldier = TacticalCreateSoldier(&CloneStruct, &iNewIndex);
										if (ClonedSoldier != NULL)
										{
											AddSoldierToSector(iNewIndex);
											//In case if it is crowded the system will auto place soldier elsewhere!
											ClonedSoldier->sGridNo = FindGridNoFromSweetSpotThroughPeople(MercPtrs[ubAttacker], pSoldier->sGridNo, 9, &pSoldier->ubDirection);
										}
									}
								//Remove the body
								TacticalRemoveSoldier(pSoldier->ubID);
		...
		//We only make corpse if soldier is not an elite or if it is an elite it should have less than or equal to 10 hp.
